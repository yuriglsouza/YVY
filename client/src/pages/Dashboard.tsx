  ...
  return (
    <div className="min-h-screen bg-background flex">
      ...
      <main className="flex-1 ml-64 p-8 lg:p-12 overflow-y-auto">
        ...
        {farms?.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-[60vh] text-center border-2 border-dashed border-border rounded-3xl bg-card/50">
            ...
            {farms?.map((farm, idx) => (
              ...
              >
                <Link href={`/farms/${farm.id}`} className="block group h-full">
                  <div className="bg-white rounded-2xl border border-border/60 overflow-hidden hover:shadow-xl hover:border-primary/50 transition-all duration-300 h-full flex flex-col">
                    <div className="h-48 bg-muted relative overflow-hidden">
                      {/* Placeholder for map preview or satellite image */}
                      {farm.imageUrl ? (
                        <img 
                          src={farm.imageUrl} 
                          alt={farm.name}
